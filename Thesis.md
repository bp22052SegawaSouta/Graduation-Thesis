...
## 使用ツール
　本章では、本研究にて使用したツールについて説明する。
### 概要
本研究で作成するAIエージェントの中核としてGemini-2.0-flash-liteを利用し、RAGではGoogleのgemini-embedding-001を利用している。また、開発支援ツールとして、RAGの性能評価やプロンプトのトレース等が行えるLangSmithを利用している。

### Gemini-2.0-flash-liteについて
Gemini-2.0-flash-liteはGoogle社が開発・提供しているサービスであり、LLMの一種である。2025年11月14日現在、最新モデルとしてGemini-2.5シリーズがリリースされているが、費用対効果の面からひとつ前の世代であるGemini-2.0-flash-liteを選択した。

### RAGおよびgemini-embedding-001について
RAG(Retrieval Augmented Generation)とはLLMがプロンプトの内容をもとにデータベースを検索し、その検索結果を含めた回答を行わせるための手法である。
本来LLMは学習に用いられたデータのみをもとにして回答を行うため、そのままでは最新の情報やプライベートな情報を含んだ質問には正確に回答を行うことができない。そこで、プロンプト上でそういった情報を追加することによって、LLMが本来知らない情報をもとに回答を行うことができるようになる。しかし、トークン数等の問題でプロンプト上に導入したいすべての情報を導入することができない場合がある。そこで、RAGを用いてベクターデータベース等を作成し、LLMはプロンプトをもとに作成したデータベースを検索し、もともと持っている情報とデータベースでの検索結果をもとに回答をおこなう。

### LangSmithについて
LangSmithとは「LLMの実行ログの収集」「LLMの出力結果のデータセット化」「登録したデータセットによるモデルの評価」の３つが行えるプラットフォームである。このツールによりエージェント内で実行したプロンプトに対してどのような出力が返ってきているのかや、RAGの性能評価等が行えるようになる。本研究では、主に各ステップのプロンプトの追跡及びRAGの性能評価において利用した。
